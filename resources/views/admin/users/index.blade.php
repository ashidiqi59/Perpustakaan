...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200/80 mb-6 overflow-hidden">
        ...
        <div class="p-4 sm:p-5">
            <form id="users-search-form" action="{{ route('admin.users.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3 items-center">
                <!-- Keep selected role -->
                <input type="hidden" id="admin-users-role" name="role" value="{{ $role }}">
    ...
    <!-- USERS TABLE -->
    <div id="users-table-wrapper" class="bg-white rounded-xl shadow-sm border border-slate-200/80 overflow-hidden">
        @if($users->count() > 0)
...
<script>
    (function() {
        ...
        var roleInput = document.getElementById('admin-users-role');
        ...
        function updateResetVisibility() {
            var hasFilter = (searchInput && searchInput.value.trim().length > 0) || (roleInput && roleInput.value !== 'all');
            if (resetContainer) {
        ...
        function performSearch(pageUrl) {
            ...
            if (pageUrl) {
                ...
                url = new URL('{{ route('admin.users.index') }}', window.location.origin);
                var q = searchInput ? searchInput.value.trim() : '';
                var r = roleInput ? roleInput.value : 'all';
                if (q) url.searchParams.set('search', q);
                if (r && r !== 'all') url.searchParams.set('role', r);
            }
            ...
            .then(function(html) {
                ...
                var newWrapper = doc.getElementById('users-table-wrapper');
