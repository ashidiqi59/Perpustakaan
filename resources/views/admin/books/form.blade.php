...
<html lang="id">
    <head>
        ...
        <title>Perpustakaan | {{ $action == 'create' ? 'Tambah Buku' : 'Edit Buku' }}</title>
        ...
        <div class="flex h-screen">
            ...
            <main class="flex-1 flex flex-col overflow-hidden">
                ...
                <header class="bg-white shadow-sm px-6 py-4 flex justify-between items-center">
                    <div>
                        <h2 class="text-xl font-semibold text-slate-800">{{ $action == 'create' ? 'Tambah Buku Baru' : 'Edit Buku' }}</h2>
                        <p class="text-sm text-slate-500">{{ $action == 'create' ? 'Tambahkan buku baru ke koleksi' : 'Perbarui informasi buku' }}</p>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-6">
                    ...
                    <form action="{{ $action == 'create' ? route('admin.books.store') : route('admin.books.update', $book->id) }}" 
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div class="lg:col-span-2 space-y-6">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label class="block text-sm font-medium text-slate-600 mb-1">ISBN *</label>
                                            <input type="text" name="isbn" value="{{ old('isbn', $book->isbn) }}" required
                                                class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                placeholder="978-1234567890">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-slate-600 mb-1">Judul Buku *</label>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select name="category" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-slate-600 mb-1">Bahasa</label>
                                            <select name="language" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label class="block text-sm font-medium text-slate-600 mb-1">Nomor Rak</label>
                                            <input type="text" name="shelf_number" value="{{ old('shelf_number', $book->shelf_number) }}"
                                                class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                placeholder="A-1-1">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-slate-600 mb-1">Jumlah Stok *</label>
                                ...
                                <div class="space-y-6">
                                    <!-- Cover Image -->
                                    <div>
                                        <label class="block text-sm font-medium text-slate-600 mb-1">Cover Buku</label>
                                        <div class="border-2 border-dashed border-slate-300 rounded-lg p-4 text-center hover:border-blue-500 transition-colors">
                                            <input type="file" name="image" id="image" accept="image/*" class="hidden"
                                                onchange="previewImage(event)">
                                            <label for="image" class="cursor-pointer">
                                                @if($book->image)
                                                    <img id="image-preview" src="{{ asset($book->image) }}" alt="Preview" class="w-full h-48 object-cover rounded-lg">
                                                @else
                                                    <img id="image-preview" src="{{ asset('images/books/spine&cover.jpg') }}" alt="Preview" class="w-full h-48 object-cover rounded-lg">
                                                @endif
                        ...
                        <div class="px-6 py-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
                            <a href="{{ route('admin.books.index') }}" class="px-6 py-2 bg-slate-500 text-white rounded-lg hover:bg-slate-600 transition-colors">
                                Batal
                            </a>
